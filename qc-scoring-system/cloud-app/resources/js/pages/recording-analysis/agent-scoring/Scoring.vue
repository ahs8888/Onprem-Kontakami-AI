<template>
    <AppLayout title="Detail Agent Scoring" :back="route('setup.recording-analysis.agent-scoring.show', scoring_uuid)">
        <section class="mx-auto rounded-lg border bg-white px-5 pb-5 md:max-w-[70%] md:px-0">
            <div class="flex items-center justify-between border-b px-6 py-3">
                <h2 class="font-krub-bold text-[15px]">{{ scoring.agent }} - {{ scoring.spv }}</h2>
                <ButtonExport
                    :action="route('setup.recording-analysis.agent-scoring.export-analysis', [scoring_uuid, item_uuid])"
                    label="Export to Excel"
                />
            </div>
            <div class="flex flex-col gap-7 px-6 py-5">
                <div class="text-center">
                    <p class="mb-[-10px] text-[12px]">Recording Score</p>
                    <h2
                        class="font-krub-bold text-[70px]"
                        v-bind:class="{
                            'text-red': scoring.avg_score <= 50,
                            'text-yellow-600': scoring.avg_score > 50 && scoring.avg_score <= 75,
                            'text-success': scoring.avg_score > 75,
                        }"
                    >
                        {{ Math.round(scoring.avg_score) }}%
                    </h2>
                </div>
                <ScoringItem :scorings="scoring.scoring" />
                <ScoringSummary :summary="scoring.summary" />
            </div>
        </section>
    </AppLayout>
</template>
<script setup lang="ts">
import ButtonExport from '@/components/button/ButtonExport.vue';
import ScoringItem from '@/components/module/scoring/ScoringItem.vue';
import ScoringSummary from '@/components/module/scoring/ScoringSummary.vue';
import AppLayout from '@/layouts/AppLayout.vue';

defineProps<{
    scoring_uuid: string;
    item_uuid: string;
    scoring: any;
}>();
</script>
