<template>
    <div x-data="{filter:false}">
        ...
        <FilterPopup @close="close" :reset="reset" classElement="md:max-w-lg">
            ...
            <SelectMultiple label="Status" name="status" placeholder="Select Status" v-model="filter.status" :items="statusList"/>
...
const close = () => {
    filter.value.date_start = ''
    filter.value.date_end = ''
    filter.value.status = []
}
</script>
